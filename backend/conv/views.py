...
class ConvertCurrency(APIView):
    ...
    def update_rates_if_necessary(self):
        try:
            last_updated = UpdateLog.objects.latest('update_at')

        except UpdateLog.DoesNotExist:
            last_updated = None

        if not last_updated or last_updated.update_at < timezone.now() - timedelta(hours=UPDATE_INTERVAL):
            self.update_rates()
    def update_rates(self):
        ...
        if 'rates' not in data:
            raise Exception('could not fatch rates')
        for code, rate in data['rates'].items():
            Currency.objects.update_or_create(code=code, defaults={'rate':rate})

        UpdateLog.objects.update_or_create(id=1, defaults={'update_at': timezone.now()})
    ...
    def perform_conversion(self,from_currency, to_currency,amount):
        try:
            ...
            return Response({'error': 'invalid Currency'}, status=status.HTTP_404_BAD_REQUEST)
